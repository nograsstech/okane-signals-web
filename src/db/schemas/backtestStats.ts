...
export const backtestStats = pgTable(
	"backtest_stats",
	{
	...
	(table) => [
		// Supports the DISTINCT ON (ticker, strategy, period, interval) query in
		// /api/strategy/list — lets Postgres resolve it with an index scan instead
		// of a full-table sort. The DESC on created_at ensures the latest row per
		// group is picked first, matching the ORDER BY in the DISTINCT ON query.
		index("idx_backtest_stats_dedup").on(
			table.ticker,
			table.strategy,
			table.period,
			table.interval,
