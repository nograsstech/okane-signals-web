...
function StrategyBacktestPage() {
	...
	const [selectedInterval, setSelectedInterval] = useState<string>(strategy?.interval || ""); // Default interval
	...
	const tradeMetrics = useMemo(() => {
		const tradeActions =
			...
			[];
		return calculateTradeMetrics(tradeActions, 10000); // Assuming $10,000 initial capital
	}, [tradeActionsData]);
					...
					<div className="border-border/50 bg-background flex-1 overflow-hidden rounded-lg border">
						{isLoading ? (
							...
							<TradingChart
								...
								intervalOptions={["1m", "5m", "15m", "30m", "1h", "4h", "1d", "1w"]}
							/>
						)}
					</div>
